...
export class NotarizationService {
  ...
  private async initializeClients(): Promise<void> {
    ...
    this.notarizationClient = await NotarizationClient.create(this.notarizationReadOnly, signer);
  ...
  async createLockedNotarization(
    ...
    if (deleteLock?.unlockAt) {
      ...
      console.log(
        `🔒 Setting delete lock: UnlockAt ${new Date(deleteLock.unlockAt * 1000).toISOString()}`,
      );
  ...
  async updateState(notarizationId: string, content: string, metadata: string): Promise<any> {
    ...
    const result = await client.updateState(state, notarizationId).buildAndExecute(client);
  ...
  async updateMetadata(notarizationId: string, metadata: string | undefined): Promise<any> {
    ...
    const result = await client.updateMetadata(metadata, notarizationId).buildAndExecute(client);
  ...
  async destroyNotarization(notarizationId: string): Promise<any> {
    ...
    const result = await client.destroy(notarizationId).buildAndExecute(client);
